...
public abstract class ApkSignerV2 {
    ...
    public static ByteBuffer[] sign(
        ...
        inputApk.clear();
        ByteBuffer beforeCentralDir = getByteBuffer(inputApk, centralDirOffset);//中央目录之前的内容
        ByteBuffer centralDir = getByteBuffer(inputApk, eocdOffset - centralDirOffset);//中央目录的内容
        ...
        ByteBuffer eocd = ByteBuffer.wrap(eocdBytes);//EOCD的内容
        eocd.order(inputApk.order());

        // Figure which which digests to use for APK contents.
        Set<Integer> contentDigestAlgorithms = new HashSet<>();//需要的摘要算法
        ...
        // Compute digests of APK contents. 计算出每种摘要算法对应的摘要
        ...
        eocd.clear();
        ZipUtils.setZipEocdCentralDirectoryOffset(eocd, centralDirOffset);//更新EOCD中中央目录的偏移量
    ...
    private static byte[] generateApkSigningBlock(byte[] apkSignatureSchemeV2Block) {
        ...
        result.putLong(blockSizeFieldValue);//V2签名块的大小

        long pairSizeFieldValue = 4 + apkSignatureSchemeV2Block.length;
        result.putLong(pairSizeFieldValue);//ID为APK_SIGNATURE_SCHEME_V2_BLOCK_ID的ID-Value的长度
        result.putInt(APK_SIGNATURE_SCHEME_V2_BLOCK_ID);//签名块对应的ID
        result.put(apkSignatureSchemeV2Block);

        result.putLong(blockSizeFieldValue);
        result.put(APK_SIGNING_BLOCK_MAGIC);//V2签名块的魔数
    ...
    private static byte[] generateSignerBlock(
        ...
        PublicKey publicKey = signerConfig.certificates.get(0).getPublicKey();//得到公钥
        ...
        signedData.digests = digests;//和签名算法对应的摘要
        ...
        for (int signatureAlgorithm : signerConfig.signatureAlgorithms) {
            ...
            byte[] signatureBytes;//对第一部分进行签名后的内容
            try {
                Signature signature = Signature.getInstance(jcaSignatureAlgorithm);//根据签名算法名，获取对应的签名算法
                signature.initSign(signerConfig.privateKey);//设置私钥
                if (jcaSignatureAlgorithmParams != null) {
                    signature.setParameter(jcaSignatureAlgorithmParams);//设置签名算法参数
                }
                signature.update(signer.signedData);//// TODO: 17/1/17 这里原来是对第一部分整体签名啊
            ...
            try { //这里主要是针对私钥签名后的数据，然后用公钥进行解密验证
    ...
    private static final class V2SignatureSchemeBlock {
        private static final class Signer {
            public byte[] signedData; //apk摘要数据（第一部分）
            public List<Pair<Integer, byte[]>> signatures;//签名数据（第二部分）（签名算法ID ：对第一部分签名后的数据）
            public byte[] publicKey;//公钥（第三部分）
        }

        private static final class SignedData {
            public List<Pair<Integer, byte[]>> digests;//不同算法的对应的APK摘要
            public List<byte[]> certificates;//证书链
        }
    }
    ...
    //生成length-prefixed sequence of certificates
    private static byte[] encodeAsSequenceOfLengthPrefixedElements(List<byte[]> sequence) {
        return encodeAsSequenceOfLengthPrefixedElements(
                sequence.toArray(new byte[sequence.size()][]));
    }
    // 生成某一个SignerConfig对应的签名数据块的摘要部分（即第一部分：length-prefixed signed data） or 生成最终的ID为0x7109871a的多个签名信息链
    ...
    // 生成length-prefixed sequence of length-prefixed digests
