...
// */
//
//package com.leon.utils.verifier;
//
...
//                findApkSigningBlock(apk, centralDirOffset);//找到V2签名块的内容和偏移量
...
//        }
//        ByteBuffer signers; //真正的签名块，可能包含了多个签名
//        try {
...
//            signerCount++;
//            try {
//                ByteBuffer signer = getLengthPrefixedSlice(signers);//具体的某个签名
...
//        ByteBuffer signedData = getLengthPrefixedSlice(signerBlock);//签名块的摘要数据
//        ByteBuffer signatures = getLengthPrefixedSlice(signerBlock);//签名块的签名数据
//        byte[] publicKeyBytes = readLengthPrefixedByteArray(signerBlock);//公钥
//
...
//            signatureCount++;
//            try {
//                ByteBuffer signature = getLengthPrefixedSlice(signatures);//getLengthPrefixedSlice方法一次性读出了一个签名的所有数据（签名算法ID和签名数据）
//                if (signature.remaining() < 8) {
//                    throw new SecurityException("Signature record too short");
//                }
//                int sigAlgorithm = signature.getInt();//签名算法ID
...
//        }
//
//        // Signature over signedData has verified. todo 看来首先是通过签名数据校验摘要本身是否被修改，然后才是计算apk摘要，并校验apk本身是否被修改
//
...
//        }
//
//        if (!signaturesSigAlgorithms.equals(digestsSigAlgorithms)) { //对比两个签名算法ID列表
//            throw new SecurityException(
...
//    }
//    // 根据计算出apk的摘要和签名块中的摘要进行对比,验证APK的完整性
...
//        }
//        byte[][] actualDigests;
//        try { //可见签名块本身不参与计算摘要，这里是计算出每个摘要算法对应的apk摘要信息
//            actualDigests =
...
//            if (!MessageDigest.isEqual(expectedDigest, actualDigest)) { //// TODO: 17/1/17 这里就是对比apk的数据摘要了
//                throw new SecurityException(
//                        getContentDigestAlgorithmJcaDigestAlgorithm(digestAlgorithm)
